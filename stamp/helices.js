...
function fillScene() {
	...
	var smallJobsHelix 	= createHelix(jobsDict["small"],  smallTop,  0.75*radius, tube/2, radialSegments, numSphereSegments, height, 4*arc, false);
	var mediumJobsHelix = createHelix(jobsDict["medium"], mediumTop, 0.75*radius, tube/2, radialSegments, numSphereSegments, height, 4*arc, false);
	//var largeJobsHelix 	= createHelix(jobsDict["large"],  largeTop,  0.75*radius, tube/2, radialSegments, numSphereSegments, height, 4*arc, false);
	//var allJobsHelix 	= createHelix(jobsDict["all"],    allTop, 	 0.75*radius, tube/2, radialSegments, numSphereSegments, height, 4*arc, false);
...
function createHelix( jobs, top, radius, sphereRadius, radialSegments, numSphereSegments, height, arc, clockwise )
{
	...
	for (var job of jobs){
		...
		//if (sphereList.length == 0) debugger;
		//++j;
		//if (job["Extension"]["LocalAccount"] in l) debugger;
		//if (job["Extension"]["LocalAccount"] == "TG-CTS090004") console.log(j);
	}
...
function drawWireframes(selectedSphere) {
	...
	// Create wireframe that hugs original sphere
	var wireMat   = new THREE.MeshBasicMaterial({ wireframe: true });
	var sphGeom   = new THREE.BufferGeometry().fromGeometry(new THREE.SphereGeometry( 5, numSphereSegments, numSphereSegments/2 ));

	// Loop through projects to highlight every sphere in project
	for (var key in projects) {

		var project = projects[key];
		//debugger
		for (var job of project["jobs"]) {

			// See if selected sphere is in job's sphere list
			//var sphereList = job["sphereList"];
			if (job["sphereList"] != undefined && project["name"] == selectedProjectName) {
